...
<el-card class="box-card">
    ...
    <!-- alert 提示 -->
    <el-alert title="修改商品" type="success" center show-icon class="mt"></el-alert>
    <!-- 步骤条 -->
        ...
        <el-tabs tab-position="left" v-model="active" class="mt" :before-leave="handleCheckCate" @tab-click="handleTabclick">
            <el-tab-pane label="基本信息" name="1">
                ...
                <el-form-item label="商品分类">
                    <!-- 级联选择器 需要转换成字符串 -->
                    <el-cascader v-model="selectOptions" :options="categorys" clearable :props="defaultProps">
                    </el-cascader>{{selectOptions}}
                </el-form-item>
            </el-tab-pane>
            <el-tab-pane label="商品参数" name="2">
                ...
                <el-form-item :label="item.attr_name" v-for="(item,index) in categoryDyParams" :key="index">
                    <!-- 带边框的多选框 -->
                    <el-checkbox v-model="item.attr_vals" :label="item1" border v-for="(item1,index) in item.attr_vals" :key="index">
                    </el-checkbox>
                </el-form-item>
            </el-tab-pane>
            ...
            <el-tab-pane label="商品内容" name="5">
                ...
                <el-button type="primary" @click="handleEditGoods">修改商品</el-button>
            </el-tab-pane>
        </el-tabs>
    </el-form>
</el-card>
    ...
    created() {
        ...
        .then(res=>{
            ...
            if(status === 200){
                ...
                //处理分类参数
                this.selectOptions = data.goods_cat.split(',').map(x => parseInt(x))
            }else{
    ...
    methods: {
        ...
        handleEditGoods(){
            ...
            var arr1 = this.categoryDyParams.map(item=>{
                return{"attr_id":item.attr_id,"attr_value":item.attr_value===''?'':item.attr_vals.join(',')}
            })
        ...
        handleAvatarSuccess(res,file){
            ...
            if(status === 200){
                this.$message.success(msg)
                //把图片添加到表单对象中
                this.formdata.pics.push({pic:data.tmp_path})
            }else{
                this.$message.error(msg)
            }
        },
        ...
        handleRemove(file,fileList){
            ...
            this.formdata.pics.forEach((item,index)=>{
                if(item.pic === file.response.data.tmp_path){
                    ...
                    this.formdata.pics.split(index,1)
                }
            })
        },
        //标签被选中
        async handleTabclick(tab) {
            ...
            if (tab.name === '2') {
              ...
              console.log(this.categoryStaticParams)
            }
        },
        //验证是否选择分类
        handleCheckCate() {
